...
void TestResults::OnTestFailure(TestDetails const& test, char const* failure)
{
    ...
    Failure temp;
    temp.file = test.filename;
    temp.lineNr = test.lineNumber;
    temp.test = test.testName;
    temp.suite = test.suiteName;
    m_failures.push_back(temp);
}
...
const std::vector<TestResults::Failure>& TestResults::GetFailures() const
{
  return m_failures;
}
