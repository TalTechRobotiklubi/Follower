...
uint16_t swap16(uint16_t v) { return ((v & 0xFF) << 8) | ((v & 0xFF00) >> 8); }

void fill_bodypos_message(float x1, float y1, float x2, float y2,
                          uint8_t* dest) {
  uint16_t payload[4] = {
      (uint16_t)clampf(x1, 0.0f, 512.f), (uint16_t)clampf(y1, 0.0f, 424.f),
      (uint16_t)clampf(x2, 0.0f, 512.f), (uint16_t)clampf(y2, 0.0f, 424.f)};
