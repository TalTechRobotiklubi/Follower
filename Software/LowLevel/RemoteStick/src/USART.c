...
GPIO_IdDef UARTx_GPIO_init(USART_TypeDef *uartX)
{
	...
	if (uartX == USART2)
	{
		...
		ioTx = USART2_TX;
		//Enable GPIO clock
		RCC_AHBPeriphClockCmd(GPIO_table[ioRx].clk, ENABLE);
	...
	while (tempTail != tempHead)
	{
		switch (rxState->analyseState)
		{
			...
			case idOK:
				...
				rxState->message.interface = InterfaceUART;
				__disable_irq();
...
{
	InterfaceHandler_transmitData(InterfaceUART, sendMessageToUSART2);
}
